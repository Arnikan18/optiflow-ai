...
async def initialize_database() -> None:
    ...
    if get_settings().seed_on_startup:
        async with db_session.async_session() as session:
            await seed_communication_if_empty(session)
...
def create_app(*, initialize_on_startup: bool = True) -> FastAPI:
    ...
    application = FastAPI(
        title="OptiFlow Communication Service",
        version="4.0",
        lifespan=lifespan,
    )
    ...
    @application.exception_handler(CommunicationError)
    async def communication_error_handler(_: Request, exc: CommunicationError) -> JSONResponse:
        return _error_json(exc.status_code, exc.message, exc.error_code)

    @application.exception_handler(RequestValidationError)
    async def validation_error_handler(_: Request, exc: RequestValidationError) -> JSONResponse:
        logger.info("Communication request validation failed: %s", exc.errors())
        return _error_json(
    ...
    async def http_error_handler(_: Request, exc: HTTPException) -> JSONResponse:
        error_code = (
            ...
            else f"COMMUNICATION_{exc.status_code}"
        )
        public_message = "Communication service error" if exc.status_code >= 500 else str(exc.detail)
        return _error_json(exc.status_code, public_message, error_code)

    @application.exception_handler(Exception)
    async def unexpected_error_handler(_: Request, exc: Exception) -> JSONResponse:
        logger.exception("Unexpected Communication service error: %s", exc)
        return _error_json(
            status.HTTP_500_INTERNAL_SERVER_ERROR,
            "Communication service encountered an unexpected error",
            "COMMUNICATION_500",
        )
    ...
    async def readiness():
        try:
            ...
            logger.exception("Communication readiness database failure: %s", exc)
            return _error_json(
                status.HTTP_503_SERVICE_UNAVAILABLE,
                "Communication database is not reachable",
                "COMMUNICATION_503",
            )
